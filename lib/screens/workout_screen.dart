  ...
  void _showFinishDialog(AppState state) {
    ...
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text('Finish Workout?', style: TextStyle(fontFamily: 'Poppins', fontWeight: FontWeight.w700, color: th.textPrimary)),
        content: Text(
          'You\'ve completed ${state.completedExerciseCount} of ${state.activeExercises.length} exercises and burned ~${state.estimatedCaloriesBurned} kcal.',
          style: TextStyle(fontFamily: 'Poppins', color: th.textSecondary, fontSize: 14),
        ),
        actions: [
          TextButton(
            onPressed: () { Navigator.pop(context); _resumeTimer(state); },
            child: Text('Continue', style: TextStyle(fontFamily: 'Poppins', color: th.textMuted)),
          ),
          ElevatedButton(
            onPressed: () { Navigator.pop(context); state.stopWorkout(); setState(() => _isPaused = false); },
            style: ElevatedButton.styleFrom(minimumSize: const Size(100, 40), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))),
      ...
      slivers: [
        SliverAppBar(
          ...
          actions: [
            TextButton(
              ...
              child: const Text('End', style: TextStyle(fontFamily: 'Poppins', color: AppTheme.accentOrange, fontWeight: FontWeight.w600)),
            ),
          ],
        ),
        ...
        SliverList(
          delegate: SliverChildBuilderDelegate(
                (ctx, i) => _ExerciseTile(exercise: state.activeExercises[i], index: i, th: th, onToggle: () => state.toggleExercise(i)),
                      ...
                      Expanded(
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          ...
                          Wrap(children: [
                            ...
                            _PlanChip(text: '${plan.exerciseCount} exercises', color: plan.color),
                            const SizedBox(width: 8),
        ...
        child: Row(children: [
          ...
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Text(exercise.muscleGroup, style: TextStyle(fontFamily: 'Poppins', fontSize: 12, color: th.textMuted)),
            ]),
          ),
  ...
  Widget build(BuildContext context) {
    return Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      ...plans.map((p) => ListTile(
        ...
        subtitle: Text('${p.duration} · ${p.exerciseCount} exercises', style: TextStyle(fontFamily: 'Poppins', fontSize: 12, color: th.textMuted)),
